...
public:
	...
	AkString(const AkString<TAlloc, T_CHAR>& in_other) { tImpl::Set(in_other.Get()); TAlloc::operator=(in_other);	}

	template<typename TAlloc2, typename T_CHAR2>
	AkString(const AkString<TAlloc2, T_CHAR2>& in_other) { tImpl::Set(in_other.Get()); TAlloc::operator=(in_other); } // if this does not compile it means that both allocation policies are not compatible.
	...
	void Transfer(AkString<TAlloc, T_CHAR>& in_from)
	{
		...
		TAlloc::TransferMem((void*&) tData::pStr, in_from, (void*)in_from.tData::pStr);
		in_from.tData::pStr = nullptr;
		
		tData::bOwner = in_from.tData::bOwner;
	...
	{
		tImpl::Set(in_rhs.Get());
		TAlloc::operator=(in_rhs);	// if this does not compile it means that both allocation policies are not compatible.
		return *this;
	}
